...
function getCoinPrice() {
	global.support.https_get("https://api.coinmarketcap.com/v2/ticker/1/?convert=EUR", function (res) {
		if (res != null && res instanceof Object && res.length !== 1 && typeof(res.data.quotes.USD) !== 'undefined' && typeof(res.data.quotes.EUR) !== 'undefined') {
			btc_usd = parseFloat(res.data.quotes.USD.price).toFixed(1);
			btc_eur = parseFloat(res.data.quotes.EUR.price).toFixed(1);
			btc_24h = parseFloat(res.data.quotes.USD.percent_change_24h).toFixed(1);
		}
	});
	global.support.https_get("https://api.coingecko.com/api/v3/coins/electronero", function (res) {
		if (res != null && res instanceof Object && typeof(res.market_data) !== 'undefined' && typeof(res.market_data.current_price.usd) !== 'undefined') {
			price_btc = parseFloat(res.market_data.current_price.btc);
			price_usd = parseFloat(res.market_data.current_price.usd);
			price_eur = parseFloat(res.market_data.current_price.eur);
			price_24h = parseFloat(res.market_data.price_change_percentage_24h).toFixed(1);
		}
	});
	return ;
}
...
function updatePoolStats2(poolType) {
    ...
    async.series([
        ...
        function (callback) {
			return callback(null, { btc: price_btc, usd: price_usd, eur: price_eur, change24h : price_24h, btc_usd : btc_usd, btc_eur : btc_eur, btc_change24h : btc_24h });
        },
        function (callback) {
            let currentEfforts = {};
...
function updateBlockHeader() {
    ...
    for (let port in min_block_rewards) {
        global.coinFuncs.getPortInfoLastBlockHeader(port, function(err, body){
            if (err !== null) {
                console.error("Last block header request failed for " + port + " port!");
            ...
            if (port == global.config.daemon.activePort) {
                info.difficulty  = body.difficulty;
                info.hash        = body.hash;
...
setInterval(getCoinPrice, 3*60*1000); // every 3 min
