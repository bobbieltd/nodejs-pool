...
let currCoinHashFactor  = {}; // coin key
...
function templateUpdateReal(coin, activePort, coinHashFactor, isHashFactorChange) {
    global.coinFuncs.getPortBlockTemplate(activePort, function (rpcResponse) {
        ...
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            rpcResponse.isHashFactorChange = isHashFactorChange;
...
function setNewCoinHashFactor(isHashFactorChange, coin, coinHashFactor, check_height) {
    ...
    if (coin !== "") set_hash_factor(coin, coinHashFactor); // used in miner.selectBestCoin
    ...
    if (isHashFactorChange) {
        ...
        if (cluster.isMaster) console.log(threadName + "Full BT update for coin " + coin + " with hash factor changed to " + coinHashFactor);
        ...
        if (cluster.isMaster) console.log(threadName + "Fast BT update for coin " + coin + " with the same " + coinHashFactor + " hash factor");
        if (check_height) {
            for (var [minerId, miner] of activeMiners) {
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.selectBestCoin = function() {
        ...
        if (typeof(this.curr_coin) !== 'undefined' && this.curr_coin_time && currCoinHashFactor[this.curr_coin] &&
        ...
        COINS.forEach(function(coin) {
            ...
            if (!(coin in activeBlockTemplate)) {
              if (miner.debugMiner) console.log(threadName + miner.logString + ": " + coin + ": no activeBlockTemplate");
              return;
            }
            const coinHashFactor = currCoinHashFactor[coin];
            if (!coinHashFactor) {
            ...
            const bt            = activeBlockTemplate[coin];
            const port          = bt.port;
        ...
        function submit_block(retry) {
            global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
                if (rpcResult.error) {
                    ...
                    let coin = portToCoin[blockTemplate.port]; // need to add portToCoin array
                    templateUpdateReal(coin, blockTemplate.port, currCoinHashFactor[coin], false); // update BT immediately
