...
let lastCoinHashFactor = {};  // coin key
let activeBlockTemplate = {}; // coin key
...
function messageHandler(message) {
    switch (message.type) {
        ...
        case 'newCoinHashFactor':
            ...
            setNewCoinHashFactor(message.data.coin, message.data.coinHashFactor);
            break;
        case 'minerPortCount':
...
function retargetMiners() {
    ...
    if (elapsed > 500) console.error("retargetMiners() consumed " + elapsed + " ms for " + activeMiners.size + " miners");
...
function checkAliveMiners() {
    ...
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("checkAliveMiners() consumed " + elapsed + " ms for " + activeMiners.size + " miners");
}
    ...
    global.support.getActivePort(coin, function (newActivePort) {
        ...
        if (newActivePort === null) {
            if (coin === "" && oldActivePort != global.config.daemon.port) {
                ...
                global.config.daemon["coinHashFactor" + coin] = 0.0;
            }
        } else {
            if (coin !== "") {
                global.support.getCoinHashFactor(coin, function (newCoinHashFactor) {
                    if (newCoinHashFactor === null) {
                        ...
                        if (newCoinHashFactor == 0) debug("Got zero " + "coinHashFactor" + coin);
                        global.config.daemon["coinHashFactor" + coin] = newCoinHashFactor;
...
function templateUpdateReal(coin, activePort, coinHashFactor) {
    global.coinFuncs.getPortBlockTemplate(activePort, function (rpcResponse) {
        ...
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            rpcResponse.coin = coin;
            rpcResponse.port = activePort;
            rpcResponse.coinHashFactor = coinHashFactor;
            ...
            setTimeout(templateUpdateReal, 3000, coin, activePort);
        }
    });
}
...
function coinHashFactorUpdate(coin, coinHashFactor) {
    ...
    if (cluster.isMaster) {
        ...
        setNewCoinHashFactor(coin, coinHashFactor);
        console.log('[*] New ' + coin + ' coin hash factor is set to ' + coinHashFactor);
    } else {
        setNewCoinHashFactor(coin, coinHashFactor);
    }
}
...
function templateUpdate(coin, repeating) {
    let activePort = global.config.daemon["activePort" + coin];
    if (activePort) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        ...
        if (err === null) {
            const coinHashFactor = coin === "" ? 1.0 : global.config.daemon["coinHashFactor" + coin];
            if (!(coin in lastBlockHash) || body.hash !== lastBlockHash[coin]) {
                lastBlockHash[coin] = body.hash;
                lastCoinHashFactor[coin] = coinHashFactor;
                templateUpdateReal(coin, activePort, coinHashFactor);
            } else if ( !(coin in lastCoinHashFactor) || (coinHashFactor == 0 && lastCoinHashFactor[coin] != 0) ||
                        (coinHashFactor != 0 && Math.abs(lastCoinHashFactor[coin] - coinHashFactor) / coinHashFactor > 0.05)
            ) {
                lastCoinHashFactor[coin] = coinHashFactor;
    ...
    }); else setTimeout(templateUpdate, 1000, coin, repeating);
...
function setNewCoinHashFactor(coin, coinHashFactor, check_height) {
    const time_before = Date.now();

    if (coin !== "") global.config.daemon["coinHashFactor" + coin] = coinHashFactor; // used in miner.selectBestCoin
    ...
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("setNewCoinHashFactor() consumed " + elapsed + " ms for " + activeMiners.size + " miners");
}

function setNewBlockTemplate(template) {
    const coin = template.coin;
    let isExtraCheck = false;
    if (coin in activeBlockTemplate) {
        if (activeBlockTemplate[coin].previous_hash.toString('hex') === template.prev_hash) {
            console.log(threadName + 'Ignoring duplicate block template update at height: ' + template.height + '. Difficulty: ' + template.difficulty);
            return;
        }
        activeBlockTemplate[coin].timeOutdate = Date.now() + 4*1000;
        pastBlockTemplates.enq(activeBlockTemplate[coin]);
        if (activeBlockTemplate[coin].port != template.port && global.config.pool.trustedMiners) isExtraCheck = true;
    }
    if (cluster.isMaster) {
    ...
    activeBlockTemplate[coin] = new BlockTemplate(template);
    const height = activeBlockTemplate[coin].height;

    if (coin === "" && global.config.daemon.port == activeBlockTemplate[""].port) {
        anchorBlockHeight = height;
    }

    setNewCoinHashFactor(coin, template.coinHashFactor, isExtraCheck ? height : 0);
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.email = pass_split.length === 2 && pass_split[1].includes("@") ? pass_split[1] : "";
    ...
    this.selectBestCoin = function() {
        if (typeof(this.curr_coin) !== 'undefined' && this.curr_coin_time && this.coin_perf[this.curr_coin] &&
        ...
        COINS.forEach(function(coin) {
            if (!(coin in activeBlockTemplate)) return;
            const port = activeBlockTemplate[coin].port;
            if (!global.coinFuncs.isMinerSupportPortAlgo(port, miner.algos, activeBlockTemplate[coin].buffer[0])) return;
            let coinHashFactor = global.config.daemon["coinHashFactor" + coin];
            if (miner.curr_coin === coin) coinHashFactor *= 1.05;
            if (coin in miner.coin_perf && miner.coin_perf[coin] * coinHashFactor > best_coin_perf) {
                debug(miner.logString + ": " + coin + ": " + miner.coin_perf[coin] * coinHashFactor);
                best_coin = coin;
                best_coin_perf = miner.coin_perf[coin] * coinHashFactor;
            }
        });
        if (typeof(this.curr_coin) === 'undefined' || this.curr_coin != best_coin) {
            this.curr_coin      = best_coin;
            this.curr_coin_time = Date.now();
            if (global.config.pool.trustedMiners) this.trust.check_height = activeBlockTemplate[best_coin].height;
        }
        return best_coin;
    }
    ...
    if (protoVersion === 1) {
        ...
        this.getJob = function () {
            ...
            let bt = activeBlockTemplate[coin];
            ...
            if (!this.proxy) {
                ...
                this.cachedJob = {
                    blob: blob,
                    algo: global.coinFuncs.algoShortTypeStr(bt.port, bt.buffer[0]),
                    variant: global.coinFuncs.variantValue(bt.port, bt.buffer[0]),
                    job_id: newJob.id,
                    target: target,
                    id: this.id
                };
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (activeBlockTemplate[blockTemplate.coin].idHash !== job.blockHash) {
        ...
        function submit_block(retry) {
            global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
                if (rpcResult.error) {
                    ...
                    console.error(threadName + "Error submitting block at height " + job.height + " (active block template height: " + activeBlockTemplate[blockTemplate.coin].height + ") from " + miner.logString + ", share type: " + shareType + ", valid: " + isNotifyAdmin + " error: " + JSON.stringify(rpcResult.error));
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        ...
        case 'submit':
            ...
            if (activeBlockTemplate[job.coin].idHash !== job.blockHash) {
                ...
                blockTemplate = activeBlockTemplate[job.coin];
            }
