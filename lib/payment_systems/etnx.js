...
const maxTxnValue = global.support.decimalToCoin(global.config.payout.maxTxnValue); // in coin units
const coinAddressLength = 98;
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function (amountToPay) {
        let paymentDetails = {
            destinations: [
                {
		...
		if (amountToPay) amount = amountToPay; // for maxTxnValue feature
        let fee = this.fee;
    ...
    this.makePaymentAsIntegrated = function (amountToPay) {
        let paymentDetails = {
            destinations: [
                {
                    amount: this.amount - this.fee,
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                ++ roundCount;
                let threshold = global.support.decimalToCoin(global.config.payout.walletMin); // not used
                ...
                if (payee.amount >= threshold) {
                    payee.setFeeAmount();
					if (payee.amount > maxTxnValue && payee.bitcoin === 0) { // maxTxnValue split implementation for big miner
						let amountPaid = 0;
						console.log("* Big " + payee.id + " miner to multiple split payment. Amount: "+payee.amount);
						for (let round = 0; round ++; round < (payee.amount/maxTxnValue + 1)) {
							let amountToPay = amountPaid + maxTxnValue <= payee.amount ? maxTxnValue : payee.amount - amountPaid;
							if (payee.paymentID === null && payee.address.length === coinAddressLength) {
								console.log("[++] " + payee.id + " miner to payment. Amount: " + global.support.coinToDecimal(payee.amount));
								paymentDestinations.push({amount: amountToPay, address: payee.address});
								payeeList.push(payee);
							} else if (payee.paymentID === null && payee.address.length !== coinAddressLength) {
								...
								console.log("[+] " + payee.id + " as separate payment to integrated address. Amount: " + global.support.coinToDecimal(amountToPay));
								payee.makePaymentAsIntegrated(amountToPay);
							} else {
								console.log("[+] " + payee.id + " as separate payment to payment ID address. Amount: " + global.support.coinToDecimal(amountToPay));
								payee.makePaymentWithID(amountToPay);
							}
							amountPaid += amountToPay;
						}
						if (amountPaid !== payee.amount) console.error("Debug purpose : wrong payments paid " + amountPaid + " !== initial due "+payee.amount);
					} else {
						if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === coinAddressLength) {
							...
							payeeList.push(payee);
						} else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length !== coinAddressLength && (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) || (payee.amount > threshold && custom_threshold))) {
							// Special code to handle integrated payment addresses.  What a pain in the rear.
							// These are exchange addresses though, so they need to hit the exchange payout amount.
                    ...
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > 0) {
                                ...
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " ETNX (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(totalFee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
