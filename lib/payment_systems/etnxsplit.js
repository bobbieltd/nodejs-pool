    ...
    async.waterfall([
        ...
        function (marketInfo, shapeshiftTxnData, intCallback) {
            ...
            paymentQueue.push(paymentDetails, function (body) {
                if (body.fee_list && body.fee_list[0] > 0) {
        ...
        function (marketInfo, shapeshiftTxnData, body, intCallback) {
            // body.tx_hash_list = XMR transaction hash.
            ...
            global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees, exchange_rate, exchange_name, exchange_txn_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [1, address, null, task.amount, body.tx_hash_list[0].match(hexChars)[0], global.config.payout.mixIn, global.support.decimalToCoin(marketInfo.minerFee), 1, global.support.decimalToCoin(marketInfo.rate), 'shapeshift', shapeshiftTxnData.orderId]).then(function (result) {
...
let xmrToQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        ...
        function (orderStatus, body, intCallback) {
            ...
            // Need to add transaction.
            global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees, exchange_rate, exchange_name, exchange_txn_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [1, address, null, global.support.decimalToCoin(orderStatus.xmr_amount_total), body.tx_hash_list[0].match(hexChars)[0], global.config.payout.mixIn, body.fee_list.reduce(reducer), 1, global.support.decimalToCoin(orderStatus.xmr_price_btc), 'xmrto', orderStatus.uuid]).then(function (result) {
...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    function getbalance() {
        global.support.rpcWallet("getbalance", paymentDetails, function (body) {
            ...
            let transferFunc = 'transfer_split';
            paymentDetails.get_tx_keys = true;
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee_list && body.fee_list[0] > 0) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " ETNX (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee_list.reduce(reducer)) + " = " + global.support.coinToDecimal(fee - body.fee_list.reduce(reducer)) + ") with 1st tx_hash " + body.tx_hash_list[0].match(hexChars)[0] + " and 1st tx_key " + body.tx_key_list[0]);
                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    [0, address, paymentID, amount, body.tx_hash_list[0].match(hexChars)[0], global.config.payout.mixIn, body.fee_list.reduce(reducer), 1]).then(function (result) {
                    if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
                        console.error("Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, '"
                            + address + "', '" + paymentID + "', " + amount + ", '" + body.tx_hash_list[0].match(hexChars)[0] + "', " + global.config.payout.mixIn + ", " + body.fee_list.reduce(reducer) + ", 1)"
                        );
                    ...
                    payee.tx_hash = body.tx_hash_list[0].match(hexChars)[0];
                    payee.tx_key = body.tx_key_list[0];
                    payee.trackPayment();
                });
            } else {
    ...
    this.makePaymentAsIntegrated = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee_list && body.fee_list[0] > 0) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " ETNX (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee_list.reduce(reducer)) + " = " + global.support.coinToDecimal(fee - body.fee_list[0]) + ") with 1st tx_hash " + body.tx_hash_list[0].match(hexChars)[0] + " and 1st tx_key " + body.tx_key_list[0]);
                global.mysql.query("INSERT INTO transactions (bitcoin, address, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [0, address, amount, body.tx_hash_list[0].match(hexChars)[0], global.config.payout.mixIn, body.fee_list.reduce(reducer), 1]).then(function (result) {
                    if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
                        console.error("Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, '"
                            + address + "', " + amount + ", '" + body.tx_hash_list[0].match(hexChars)[0] + "', " + global.config.payout.mixIn + ", " + body.fee_list[0] + ", 1)"
                        );
                        payee.transactionID = 0;
                        payee.manualPaymentShow();
                        full_stop(result);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_email IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s ETNX payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s ETNX (with tx fee %(fee)s ETNX) to %(address2)s wallet was just performed and total due was decreased by %(amount)s ETNX.\n" +
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                ...
                if (payee.amount >= threshold) {
                    payee.setFeeAmount();
                    if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === 98) {
                    ...
                    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length !== 98 && (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) || (payee.amount > threshold && custom_threshold))) {
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee_list && body.fee_list[0] > 0) {
                                ...
                                paymentDetails.destinations.forEach(function (payeeItem) {
                                    ...
                                    console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " ETNX (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                                });
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " ETNX (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee_list.reduce(reducer)) + " = " + global.support.coinToDecimal(totalFee - body.fee_list.reduce(reducer)) + ") with 1st tx_hash " + body.tx_hash_list[0].match(hexChars)[0] + " and tx_key " + body.tx_key_list[0]);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                    [0, null, null, totalAmount, body.tx_hash_list[0].match(hexChars)[0], global.config.payout.mixIn, body.fee_list[0], paymentDetails.destinations.length]).then(function (result) {
                                    if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
                                        console.error("Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, null, null, "
                                            + totalAmount + ", '" + body.tx_hash_list[0].match(hexChars)[0] + "', " + global.config.payout.mixIn + ", " + body.tx_key_list[0] + ", " + paymentDetails.destinations.length + ")"
                                        );
                                        paymentDetails.destinations.forEach(function (payeeItem) {
